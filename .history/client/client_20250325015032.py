...
class Client:
    ...
    def quantize_weights(self, state_dict):
        # Compress to 16-bit floats
        quantized = {k: v.to(torch.float16) for k, v in state_dict.items()}
        return quantized
    ...
    def train(self, bandwidth):
        ...
        for data, target in loader:
            ...
            loss.backward()
            optimizer.step()
        training_time = time.time() - start_time  # Actual time in seconds
        
        self.E_c = training_time  # Replace simulated E_c
        self.L_local = self.DS / (bandwidth * self.CPU + 1e-6) + training_time  # Include training time
        self.CPU = psutil.cpu_percent(interval=1) / 100
