...
from common.utils import compute_DF, setup_logging, hash_weights
...
class Client:
    def __init__(self, client_id, edge_host, edge_port):
        ...
        try:
            with open('/data/mnist_train_400.pkl', 'rb') as f:
                data = pickle.load(f)
                self.data = data[client_id]
                self.logger.info(f"Loaded {len(self.data)} samples from index {client_id}")
        ...
        self.weight_hash = 0  # Initialize weight hash
    ...
    def train(self, bandwidth, epochs=2):
        ...
        try:
            for epoch in range(epochs):
                ...
                for batch_idx, (data, target) in enumerate(loader):
                    self.logger.debug(f"Epoch {epoch + 1}/{epochs}, Batch {batch_idx + 1}/{len(loader)}")
                    optimizer.zero_grad()
        ...
        self.CPU = psutil.cpu_percent(interval=1) / 100
        self.E_c = min(0.01 * self.DS / max(self.CPU, 0.1), 10.0)  # Cap at 10
        self.L_local = min(0.1 * self.DS / (bandwidth * max(self.CPU, 0.1) + 1e-6), 100.0)  # Cap at 100
        
        v_c = torch.var(torch.tensor([t for _, t in self.data], dtype=torch.float32))
        self.logger.debug(f"Computed variance: {v_c}")
        self.DF = compute_DF(v_c, self.DS).item()
        self.weight_hash = hash_weights(self.model.state_dict())
        self.logger.info(f"Training finished in {elapsed:.2f}s, E_c: {self.E_c:.3f}, L_local: {self.L_local:.3f}, DF: {self.DF:.3f}, Weight Hash: {self.weight_hash}")

    def send_update(self):
        self.logger.info("Preparing to send update to edge")
        state_dict = self.model.state_dict()
        state = [self.DF, self.DS, self.E_c, self.CPU, self.L_local, self.weight_hash]  # 6 elements
...
if __name__ == "__main__":
    ...
    state_dict = {
        ...
        "Weight Hash": client.weight_hash
    }
