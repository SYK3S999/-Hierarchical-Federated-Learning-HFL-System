...
class Client:
    def __init__(self, client_id, edge_host, edge_port):
        ...
        self.CPU = min(psutil.cpu_percent(interval=1) / 100, 0.15)  # Cap at Docker limit
    ...
    def train(self, bandwidth):
        ...
        # Fix: Normalize L_local (assume bandwidth in MB/s, DS in samples)
        self.L_local = (self.DS * 0.001) / (bandwidth + 1e-6) + training_time  # Simplified delay
    ...
    def send_update(self):
        ...
        try:
            ...
            print(f"Client {self.id} failed to send update: {e}")
            return False
        return True
