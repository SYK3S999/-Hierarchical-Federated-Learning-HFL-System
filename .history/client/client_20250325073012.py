...
class Client:
    def __init__(self, client_id, edge_host, edge_port, epochs=1):
        try:
            ...
            logging.info(f"Client {self.id} initialized with DS={self.DS}, edge={self.edge_host}:{self.edge_port}")
        except Exception as e:
            logging.error(f"Client {client_id} initialization failed: {e}\n{traceback.format_exc()}")
            raise
    ...
    def train(self, bandwidth):
        try:
            ...
            logging.error(f"Client {self.id} training failed: {e}\n{traceback.format_exc()}")
            raise
    ...
    def send_update(self):
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.settimeout(10)
                logging.info(f"Client {self.id} attempting to connect to {self.edge_host}:{self.edge_port}")
...
if __name__ == "__main__":
    try:
        ...
        logging.info(f"Starting client {client_id} with bandwidth={bandwidth}, epochs={epochs}")
        ...
        logging.error(f"Client {client_id} main execution failed: {e}\n{traceback.format_exc()}")
        sys.exit(1)