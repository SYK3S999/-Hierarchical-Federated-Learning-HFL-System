...
class Client:
    ...
    def train(self, bandwidth):
        ...
        logging.info(f"Client {self.id} trained for {self.epochs} epochs")
    ...
    def send_update(self):
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                s.sendall(data)
                logging.info(f"Client {self.id} sent update to {self.edge_host}:{self.edge_port}")
        except Exception as e:
            logging.error(f"Client {self.id} failed to send update: {e}")
            raise
