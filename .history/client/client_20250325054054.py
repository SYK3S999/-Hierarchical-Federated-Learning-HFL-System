...
class Client:
        ...
        for attempt in range(5):
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(10)  # Increased from 5s
    ...
    def run(self, bandwidth):
        for round_num in range(self.rounds):
            ...
            time.sleep(1)  # Small delay to sync with edges

if __name__ == "__main__":
    client_id = int(sys.argv[1])
    edge_id = 0 if client_id < 3 else (1 if client_id < 6 else (2 if client_id < 8 else 3))
    edge_host = f'edge{edge_id}-1'
    edge_port = 5000 + edge_id
    
    time.sleep(8 if edge_id == 2 else 10)  # Initial delay for edge startup
    client = Client(client_id, edge_host, edge_port, rounds=5)
    client.run(float(sys.argv[2]))