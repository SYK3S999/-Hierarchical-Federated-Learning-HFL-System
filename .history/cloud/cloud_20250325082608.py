...
class Cloud:
    ...
    def listen_and_aggregate(self):
        ...
        self.weight_hash_avg = sum(update['state'][5] for update in edge_updates) / len(edge_updates)
        reward = 10.0 * (acc - old_acc) - 0.01 * E_total - 0.01 * L_avg  # Adjusted weights
        
        self.logger.info(f"Aggregation complete: DF_avg: {self.DF_avg:.3f}, DS_total: {self.DS_total}, E_total: {E_total:.3f}, L_avg: {L_avg:.3f}, Weight Hash Avg: {self.weight_hash_avg}, Reward: {reward:.4f}")
        ...
        state_dict = {
            ...
            "Weight Hash Avg": self.weight_hash_avg
        }
