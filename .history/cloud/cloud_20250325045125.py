...
class Cloud:
    ...
    def listen_and_aggregate(self):
        edge_updates = []
        edge_count = 4
        for attempt in range(3):  # Retry binding if it fails
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.bind(('0.0.0.0', self.port))
                s.listen(edge_count)
                s.settimeout(60)
                print(f"Cloud listening on 0.0.0.0:{self.port} (attempt {attempt + 1})")
                break
            except Exception as e:
                print(f"Cloud failed to bind (attempt {attempt + 1}/3): {e}")
                time.sleep(2)
        else:
            print("Cloud failed to bind after 3 attempts")
            return None, 0, 0, 0
