...
class Cloud:
    ...
    def listen_and_aggregate(self):
        ...
        # Fix: Compute E_total and L_avg from client data passed through edges
        client_E_total = sum(update['state'][2] for update in edge_updates)  # Sum of client E_c from edges
        E_total = client_E_total + self.E_c
        L_avg = sum(update['state'][4] for update in edge_updates) / len(edge_updates)  # Average of client L_local
