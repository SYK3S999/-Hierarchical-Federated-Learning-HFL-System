...
class Edge:
    ...
    def listen_and_aggregate(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.settimeout(30)  # Increased to 30s
...
if __name__ == "__main__":
    ...
    time.sleep(5)  # Wait for cloud (reduced from 10s since cloud starts first)
    try:
        edge = Edge(edge_id, 'cloud-1', 6000, int(sys.argv[2]))
        edge.listen_and_aggregate()
        logger.info(f"Edge {edge_id} - State: {edge.DF_j, edge.DS_j, edge.E_c, edge.CPU_j, edge.L_avg}")
    except Exception as e:
