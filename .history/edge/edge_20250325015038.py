...
class Edge:
    def __init__(self, edge_id, cloud_host, cloud_port, client_count):
        ...
        self.CPU_j = 0.3
        self.L_avg = 0.0
        self.peers = [5000 + i for i in range(4) if i != edge_id]  # Other edge ports
    ...
    def share_with_peers(self, data):
        for peer_port in self.peers:
            try:
                ...
                continue  # Ignore failed peer comms
    ...
    def listen_and_aggregate(self):
        ...
        for model, update in zip(models, client_updates):
            model.load_state_dict({k: v.to(torch.float32) for k, v in update['weights'].items()})  # Dequantize
...
if __name__ == "__main__":
    ...
    edge = Edge(edge_id, 'cloud', 6000, int(sys.argv[2]))  # Client count as arg
