...
class Edge:
    def __init__(self, edge_id, cloud_host, cloud_port, client_count):
        ...
        print(f"Edge {self.id} initialized, targeting {self.cloud_host}:{self.cloud_port}")
    ...
    def listen_and_aggregate(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.settimeout(15)
            print(f"Edge {self.id} listening on port {self.port}")
            try:
                for _ in range(self.client_count):
                    conn, addr = s.accept()
                    print(f"Edge {self.id} accepted connection from {addr}")
                    data = b''
                    while True:
        ...
        for attempt in range(5):  # Increased to 5 retries
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(10)  # Increased to 10s
                    print(f"Edge {self.id} attempting to connect to {self.cloud_host}:{self.cloud_port} (attempt {attempt + 1})")
                    s.connect((self.cloud_host, self.cloud_port))
                    s.sendall(data)
                print(f"Edge {self.id} successfully sent to cloud")
                return True
            except Exception as e:
                print(f"Edge {self.id} failed to send to cloud (attempt {attempt + 1}/5): {e}")
                time.sleep(2)
        print(f"Edge {self.id} gave up sending to cloud after 5 attempts")
        return False
