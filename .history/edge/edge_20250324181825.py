...
class Edge:
    ...
    def listen_and_aggregate(self):
        ...
        models = [update['weights'] for update in client_updates]
        weights = [update['DS'] for update in client_updates]
        aggregate_models(self.model, [CNN().load_state_dict(m) for m in models], weights)
        ...
        state = [self.DF_j, self.DS_j, self.E_c, self.CPU_j, self.B_edge]
...
if __name__ == "__main__":
    ...
    print(f"Edge {edge_id} - State: {edge.DF_j, edge.DS_j, edge.E_c, edge.CPU_j, edge.B_edge}")