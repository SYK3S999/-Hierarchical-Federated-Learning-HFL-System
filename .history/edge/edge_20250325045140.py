...
class Edge:
    ...
    def listen_and_aggregate(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.settimeout(20)
            print(f"Edge {self.id} listening on 0.0.0.0:{self.port}")
            try:
                for _ in range(self.client_count):
                    conn, addr = s.accept()
                    print(f"Edge {self.id} received connection from {addr}")
                    data = b''
                    while True:
        ...
        print(f"Edge {self.id} attempting to connect to {self.cloud_host}:{self.cloud_port}")
        for attempt in range(5):  # Increased to 5 attempts
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    s.sendall(data)
                print(f"Edge {self.id} sent update to cloud successfully")
                return True
