...
from common.utils import aggregate_models, compute_DF  # Updated import
...
class Edge:
    ...
    def receive_client_updates(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.settimeout(60)  # Increased from 30s
    ...
    def send_update_to_cloud(self, state, weights):
        ...
        time.sleep(2)  # Delay to sync with cloud
    ...
    def run(self):
        ...
        for round_num in range(self.rounds):
            ...
            if not updates:
                ...
                # Extract models and weights from updates
                source_models = [CNN() for _ in updates]  # Create new models for each update
                weights = [1.0] * len(updates)  # Equal weighting for now
                for i, update in enumerate(updates):
                    source_models[i].load_state_dict(update['weights'])
                
                # Aggregate models into self.model
                aggregate_models(self.model, source_models, weights)
                ...
                self.send_update_to_cloud(state, self.model.state_dict())
        ...
        time.sleep(60)  # Extra time to ensure cloud processes
